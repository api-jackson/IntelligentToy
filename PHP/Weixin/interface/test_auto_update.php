	...
	if($auto_update == "Y")
	{
		...
		while(($server_fileName = readdir($server_handler)) !== false)
		{
			if($server_fileName[0] != "."){
				...
				$server_fileName = explode(".", $server_fileName);
				...
				if($device_handler)
				{
					while(!feof($device_handler))
					{
						...
						$device_filename = explode(".", $device_filename);
						
						if($server_fileName[0] != $device_filename[0])
						{
							echo "compare: device-".$device_filename[0]."\t server-".$server_fileName[0]."\n";
							continue;
						}
...
// 					clientOutput($str);
					echo "file not exist: ".$server_fileName[0]."\n";
				}
			}
		}
	}
